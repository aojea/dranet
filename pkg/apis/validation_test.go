...
func TestValidateInterfaceConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cfg:       &InterfaceConfig{Name: "eth0", IPAM: &IPAMConfig{Ranges: []string{"10.10.0.0/24"}}},
			fieldPath: "iface",
			expectErr: false,
		},
		{
			name:      "invalid with ipam and addresses",
			cfg:       &InterfaceConfig{Name: "eth0", IPAM: &IPAMConfig{Ranges: []string{"10.10.0.0/24"}}, Addresses: []string{"10.0.0.1/24"}},
			fieldPath: "iface",
			expectErr: true,
			errCount:  1,
		},
		{
			...
			cfg:       &InterfaceConfig{Name: "eth0", IPAM: &IPAMConfig{Ranges: []string{"10.10.0.0/24"}}, DHCP: ptr.To(true)},
			fieldPath: "iface",
			expectErr: true,
			errCount:  1,
		},
		{
			name:      "invalid with empty ipam ranges",
			cfg:       &InterfaceConfig{Name: "eth0", IPAM: &IPAMConfig{}},
			fieldPath: "iface",
			expectErr: true,
			errCount:  1,
		},
		{
			name:      "invalid ipam range",
			cfg:       &InterfaceConfig{Name: "eth0", IPAM: &IPAMConfig{Ranges: []string{"bad-range"}}},
			fieldPath: "iface",
			expectErr: true,
			errCount:  1,
		},

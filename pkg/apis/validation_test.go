...
func TestValidateInterfaceConfig(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "invalid arpIgnore (reserved 4)",
			cfg:       &InterfaceConfig{Name: "eth0", ARPIgnore: ptr.To[int32](4)},
			fieldPath: "iface",
			expectErr: true,
			errCount:  1,
		},
		{
			name:      "invalid arpIgnore (reserved 5)",
			cfg:       &InterfaceConfig{Name: "eth0", ARPIgnore: ptr.To[int32](5)},
			fieldPath: "iface",
			expectErr: true,
			errCount:  1,
		},
		{
			name:      "invalid arpIgnore (reserved 6)",
			cfg:       &InterfaceConfig{Name: "eth0", ARPIgnore: ptr.To[int32](6)},
			fieldPath: "iface",
			expectErr: true,
			errCount:  1,
		},
		{
			name:      "invalid arpIgnore (reserved 7)",
			cfg:       &InterfaceConfig{Name: "eth0", ARPIgnore: ptr.To[int32](7)},
			fieldPath: "iface",
			expectErr: true,
			errCount:  1,
		},
